...
void setup() {
  ...
  WiFi.mode(WIFI_AP);
  WiFi.softAP(MB_AP_SSID, MB_AP_PASSWORD);
  Serial.printf("[wBMS-Slave] AP: %s  IP: http://%s\n", MB_AP_SSID,
  ...
  Serial.printf("[wBMS-Slave] Connect to '%s' pw '%s' -> http://%s\n", MB_AP_SSID,
                MB_AP_PASSWORD, WiFi.softAPIP().toString().c_str());
}
